...
public class ProdutoService : IProdutoService
{
    ...
    public async Task<bool> Add(ProdutoAdicionarDto produtoAdicionarDto)
    {
        var response = await _httpClient.PostAsJsonAsync("api/produtos", produtoAdicionarDto);
        return true;
    }
    ...
    public void Delete(int id)
    {
        try
        {
            var response = _httpClient.DeleteAsync($"api/produtos/{id}");
        }
        catch (Exception)
        {
            throw;
